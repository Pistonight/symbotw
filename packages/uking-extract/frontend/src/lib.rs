...
pub fn run(options: &Options) -> anyhow::Result<()> {
    ...
    let data_sheet: DataSheet =
        serde_yaml_ng::from_reader(&mut reader)?;
    ...
    main_script.push_str(&format!("    run_with_frontend({}, get_frontend(), process_imports)\n", options.verbose));
    ...
    common::ensure_parent_exists(&options.output)?;
