...
use elf::abi::{PT_LOAD, R_AARCH64_ABS64, R_AARCH64_GLOB_DAT, R_AARCH64_JUMP_SLOT, R_AARCH64_RELATIVE};
use uking_relocate_lib::ProgramRegion;
use anyhow::{anyhow, bail};

use crate::{cli::RegionArg, elf::{DynamicSymbolTables, ElfWrapper}, module::{ModuleData, ModuleInfo, ModuleType, Modules}};
...
impl Memory {
    ...
    pub fn load(start: u64, module_data: &ModuleData) -> anyhow::Result<Self> {
        ...
        let count = rtld_elf.load_dynamic_symbols(ModuleType::None, start + module_data.info.rtld.start as u64, &mut dynamic_symbols.rtld)?;
        println!();
        println!("MODULE   DYNAMIC SYMBOLS");
        println!("rtld     {}", count);
        let count = main_elf.load_dynamic_symbols(ModuleType::Main, start + module_data.info.main.start as u64, &mut dynamic_symbols.main)?;
        println!("main     {}", count);
        let count = subsdk0_elf.load_dynamic_symbols(ModuleType::Subsdk0, start + module_data.info.subsdk0.start as u64, &mut dynamic_symbols.subsdk0)?;
        println!("subsdk0  {}", count);
        let count = sdk_elf.load_dynamic_symbols(ModuleType::Sdk, start + module_data.info.sdk.start as u64, &mut dynamic_symbols.sdk)?;
        println!("sdk      {}", count);

        let mut count = 0;
        count += mem.relocate(ModuleType::None, &rtld_elf, &module_data.info.rtld, &dynamic_symbols)?;
        count += mem.relocate(ModuleType::Main, &main_elf, &module_data.info.main, &dynamic_symbols)?;
        count += mem.relocate(ModuleType::Subsdk0, &subsdk0_elf, &module_data.info.subsdk0, &dynamic_symbols)?;
        count += mem.relocate(ModuleType::Sdk, &sdk_elf, &module_data.info.sdk, &dynamic_symbols)?;
    ...
    fn load_module(
        ...
        for ph in elf.segments {
            if ph.p_type == PT_LOAD {
                ...
                if ph.p_vaddr != (segment_start - info.start) as u64 {
                    bail!("unexpected p_vaddr != start ({} != {})", ph.p_vaddr, segment_start - info.start);
                }
                let segment_data = elf.segment_data(&ph)?;
                let permission = ph.p_flags;
                let region = Region::allocate(module, segment_start, permission, segment_data, ph.p_memsz as u32);
                let size = region.get_byte_len();
                println!("{:8}0x{:08x} 0x{:08x} 0x{:08x}  {}", module.to_string(), segment_start, ph.p_memsz, size, perm_str(permission));
        ...
        if segment_start != info.end {
            bail!("unexpected end mismatch for {}, expected 0x{:08x}, actual 0x{:08x}", module, info.end, segment_start);
        }
    ...
    fn relocate(&mut self, module: ModuleType, elf: &ElfWrapper, info: &ModuleInfo, dynamic: &DynamicSymbolTables) -> anyhow::Result<u32> {
        println!("-- applying relocation to {}", module);

        let mut module_regions = self.regions.iter_mut().filter(|r| r.module == module).collect::<Vec<_>>();
        let (symbols, strtab) = elf.dynamic_symbol_table()?.ok_or_else(|| anyhow!("missing dynamic symbol table"))?;
        ...
        for rela in elf.rela_dyn()? {
            match rela.r_type {
                R_AARCH64_ABS64 => {
                    ...
                    if rela.r_sym == 0 {
                        bail!("unexpected empty r_sym in .rela.dyn: 0x{:08x}", rela.r_offset);
                    }
                    if rela.r_addend < 0 {
                        // BOTW only has positive r_addend
                        bail!("unexpected negative r_addend in .rela.dyn: {}", rela.r_addend);
                    }
                ...
                R_AARCH64_GLOB_DAT => {
                    ...
                    if rela.r_addend != 0 {
                        bail!("unexpected r_addend in .rela.dyn for GLOB_DAT: {}", rela.r_addend);
                    }
                    let symbol = symbols.get(rela.r_sym as usize)?;
                    let symbol_name = strtab.get(symbol.st_name as usize)?;
                ...
                R_AARCH64_RELATIVE => {
                    ...
                    if rela.r_sym != 0 {
                        bail!("unexpected r_sym in {}.rela.dyn: 0x{:08x}", module, rela.r_sym);
                    }
                    let offset = rela.r_offset as u32;
                    if rela.r_addend < 0 {
                        bail!("unexpected r_addend in {}.rela.dyn: 0x{:08x}", module, rela.r_addend);
                    }
        ...
        for rela in elf.rela_plt()? {
            ...
            match rela.r_type {
                R_AARCH64_JUMP_SLOT => {
                    if rela.r_sym == 0 {
                        bail!("unexpected empty r_sym in .rela.plt: 0x{:08x}", rela.r_offset);
                    }
                    if rela.r_addend != 0 {
        ...
        if !unresolved_global_data.is_empty() {
            println!("WARNING - the following global variables are unresolved: {:?}", unresolved_global_data);
        }
        if !unresolved_global_plt.is_empty() {
            println!("WARNING - the following GOT PLT entries are unresolved: {:?}", unresolved_global_data);
        }
        Ok(count)
    ...
    fn write_relocation(regions: &mut [&mut Region], offset: u32, value: u64) -> anyhow::Result<()> {
    ...
    pub fn to_program_regions(&self, regions: &[RegionArg]) -> Vec<ProgramRegion> {
        ...
        for region in regions {
            let region_start = align_down!(self.info.to_program_offset(region.module, region.start));
...
impl Region {
    ...
    pub fn allocate(module: ModuleType, start: u32, permissions: u32, data: &[u8], mem_size: u32) -> Self {
    ...
    pub fn write(&mut self, offset: u32, value: u64) {
        ...
        self.pages[page_idx].data[page_offset..page_offset+8].copy_from_slice(&value.to_le_bytes());
    }
    ...
    pub fn copy_overlapped(&self, rel_start: u32, num_pages: u32, out: &mut Vec<ProgramRegion>) {
        ...
        println!("loading 0x{:08x}-0x{:08x} {} {}", rel_start, rel_end, perm_str(self.permissions), self.module);
        ...
        out.push(ProgramRegion::new(program_rel_start, self.permissions, out_mem));
    }
}
